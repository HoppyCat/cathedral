...
2. `TRONIE.md` - optional starting impression or roleplay-only file; check status before adopting
3. `AGENTS.md` - operational role, runtime modes
4. `STYLE.md` - voice, listening, surface calibration
5. `KNOW.md` - social frameworks, companion theory, listening micro-skills
6. `MEMORY.md` - long-term memories chosen for retention
7. `WORKING_MEMORY.md` - unresolved material not yet committed to memory
8. `USER.md` or `USERS.md` - human-specific routing, private/confidential notes if provided
9. `CHANGELOG.md` - change authorship and file evolution
10. `HEURISTICS.md` - routing layer
11. `INDEX.md` - map to the extended file library
